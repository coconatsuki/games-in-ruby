class Board
  ...
  def setting_position(symbol) #ask_for_next_position
    @position = gets.chomp.upcase # This is a valid case of using a while   #print le text de non-valide ici
    if valid?(@position)
  ...
  def print_position(symbol) #play (name) @used_boxes << ...
  ...
  def valid?(x_or_o)
    if @boxes.has_key?(x_or_o)
      ...
      puts "\nI didn't get this. Could you repeat ?" #should not be here
